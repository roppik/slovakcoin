...
#if defined(NDEBUG)
# error "slovakcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf5e67aa8031028782dbd6bded378598da63297517a5c3ea9ad47cfe1b9e85b4f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // slovakcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "slovakcoin Signed Message:\n";
...
{
    // slovakcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // slovakcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // slovakcoin: 1 days
static const int64 nTargetSpacing = 2.5 * 60; // slovakcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // slovakcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // slovakcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf1df6e9944b63f0df86116384779afcf9ba93ac7b85e4ead69f08d438cdc1e45");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Roppik Passoci Slovakia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bddcc73d282f09242ad1bc8ad0069904bde5500257781920d6839b7e4d74df3b421d21ddc05a9d9ba47715f20546353901611d5b9139e76752813957642db5fd") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1546985555;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086126889;

        if (fTestNet)
        {
            block.nTime    = 1546985489;
            block.nNonce   = 386510258;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc8b92b068d56b059720db95217592be43ca8cfc7703cf61a964eaa5cbed0a554"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xca, 0xba, 0xd6 }; // slovakcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("slovakcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("slovakcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static slovakcoinMiner(CWallet *pwallet)
{
    printf("slovakcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("slovakcoin-miner");
    ...
    try { loop {
        ...
        printf("Running slovakcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("slovakcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&slovakcoinMiner, pwallet));
}
